...
const Footer = () => {
  return (
    <div className="border-t-2 border-neutral-400 py-2">
      <div className="flex flex-col md:flex-row items-center justify-center gap-2">
          <Logo />
          <div>
            <div className='grid w-full justify-evenly sm:flex md:grid-cols-2 lg:grid-cols-3 '>
                {footerData.map((data) => 
                  <div className='mt-5 flex-flex-col gap-5 text-start p-4'>
                  <h2 className="text-lg font-bold text-neutral-700">{data.title}</h2>
                  <Link to="/"><p className="text-sm mt-2">{data.link1}</p></Link>
                  <Link to="/"><p className="text-sm mt-2">{data.link2}</p></Link>           
                  </div>
                )}
            </div>
          </div>   
      ...
      <div className='w-full flex lg:flex-row flex-col items-center gap-4 justify-center border-t-[1px] border-neutral-400 mt-6 pt-6 font-semibold text-neutral-600 md:flex-row md:justify-evenly flex-shrink border-opacity-70'>
          ...
          {footerIcon.map((icon) => <a href={icon.href} className="hover:text-neutral-600 transition duration-200">{icon.icon}</a>
           )}
          </div>
        </div>
    </div>
